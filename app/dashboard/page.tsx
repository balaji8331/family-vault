...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        // Fetch total documents owned by the user
        const { count: totalDocs, error: err1 } = await supabase
          .from('documents')
          .select('*', { count: 'exact', head: true })
          .eq('owner_id', currentUser.id);

        if (err1) console.error('Error fetching total docs:', err1);

        // Fetch documents shared with the user (granted_to)
        const { count: sharedDocs, error: err2 } = await supabase
          .from('document_access')
          .select('*', { count: 'exact', head: true })
          .eq('granted_to', currentUser.id);

        if (err2) console.error('Error fetching shared docs:', err2);

        // Fetch documents expiring soon (next 30 days) using expiry_date
        const thirtyDaysFromNow = new Date();
        thirtyDaysFromNow.setDate(thirtyDaysFromNow.getDate() + 30);
        
        const { count: expiringDocs, error: err3 } = await supabase
          .from('documents')
          .select('*', { count: 'exact', head: true })
          .eq('owner_id', currentUser.id)
        ...
        if (err3) console.error('Error fetching expiring docs:', err3);

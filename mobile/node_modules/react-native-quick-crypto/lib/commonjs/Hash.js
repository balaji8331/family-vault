...
const asyncDigest = async (algorithm, data) => {
  ...
  if (name === 'SHA-1' || name === 'SHA-256' || name === 'SHA-384' || name === 'SHA-512' || name === 'SHA3-256' || name === 'SHA3-384' || name === 'SHA3-512') {
  ...
  if (name === 'cSHAKE128' || name === 'cSHAKE256') {
    ...
    if (typeof algorithm.outputLength !== 'number' || algorithm.outputLength <= 0) {
      throw (0, _errors.lazyDOMException)('CShakeParams.outputLength is required', 'OperationError');
    }
    if (algorithm.outputLength % 8) {
      throw (0, _errors.lazyDOMException)('Unsupported CShakeParams outputLength', 'NotSupportedError');
    }
  ...
  throw (0, _errors.lazyDOMException)(`Unrecognized algorithm name: ${name}`, 'NotSupportedError');
};
...
const getTurboShake = () => {
  if (!nativeTurboShake) {
    nativeTurboShake = _reactNativeNitroModules.NitroModules.createHybridObject('TurboShake');
  }
...
const turboShakeDigest = async (name, algorithm, data) => {
  if (typeof algorithm.outputLength !== 'number' || algorithm.outputLength <= 0) {
    throw (0, _errors.lazyDOMException)('TurboShakeParams.outputLength is required', 'OperationError');
  }
  if (algorithm.outputLength % 8) {
  ...
  return getTurboShake().turboShake(name, ds, algorithm.outputLength / 8, (0, _utils.bufferLikeToArrayBuffer)(data));
};
const kangarooTwelveDigest = async (name, algorithm, data) => {
  if (typeof algorithm.outputLength !== 'number' || algorithm.outputLength <= 0) {
    throw (0, _errors.lazyDOMException)('KangarooTwelveParams.outputLength is required', 'OperationError');
  }
  if (algorithm.outputLength % 8) {
    throw (0, _errors.lazyDOMException)('Invalid KangarooTwelveParams outputLength', 'OperationError');
  }
  const customization = algorithm.customization !== undefined ? (0, _utils.bufferLikeToArrayBuffer)(algorithm.customization) : undefined;
  return getTurboShake().kangarooTwelve(name, algorithm.outputLength / 8, (0, _utils.bufferLikeToArrayBuffer)(data), customization);
};

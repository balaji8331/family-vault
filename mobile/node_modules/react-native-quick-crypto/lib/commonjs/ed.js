...
function resolveDhKeyInput(input, name) {
  if (isRawKeyInput(input)) {
    ...
    const keyData = (0, _utils.binaryLikeToArrayBuffer)(input.key);
...
function ed_formatKeyPairOutput(ed, encoding) {
  ...
  if (publicFormat == null || publicFormat === -1) {
    ...
    publicKey = _reactNativeBuffer.Buffer.from(pub.handle.exportRawPublic());
  } else {
  ...
  if (privateFormat == null || privateFormat === -1) {
    ...
    privateKey = _reactNativeBuffer.Buffer.from(priv.handle.exportRawPrivate());
  } else if (privateFormat === 'raw-seed') {
    privateKey = _reactNativeBuffer.Buffer.from(priv.handle.exportRawSeed());
  } else {

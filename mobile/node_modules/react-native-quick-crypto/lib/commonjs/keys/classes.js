...
class CryptoKey {
  ...
  constructor(keyObject, keyAlgorithm, keyUsages, keyExtractable) {
    ...
    this.keyUsages = Object.freeze((0, _utils.getSortedUsages)(keyUsages));
...
class KeyObject {
  ...
  static createKeyObject(type, key, format, encoding, passphrase) {
    if (type !== 'secret' && type !== 'public' && type !== 'private') throw new Error(`invalid KeyObject type: ${type}`);
  ...
  export(options) {
    ...
    if (options?.format === 'raw-public') {
      if (this.asymmetricKeyType === 'ec') {
        ...
        return _reactNativeBuffer.Buffer.from(this.handle.exportECPublicRaw(pointType === 'compressed'));
      }
      return _reactNativeBuffer.Buffer.from(this.handle.exportRawPublic());
    }
    const {
      format,
      type
    ...
    const buffer = _reactNativeBuffer.Buffer.from(key);
...
class PrivateKeyObject extends AsymmetricKeyObject {
  ...
  export(options) {
    ...
    if (options?.format === 'raw-private') {
      if (this.asymmetricKeyType === 'ec') {
        return _reactNativeBuffer.Buffer.from(this.handle.exportECPrivateRaw());
      }
      return _reactNativeBuffer.Buffer.from(this.handle.exportRawPrivate());
    }
    if (options?.format === 'raw-seed') {
      return _reactNativeBuffer.Buffer.from(this.handle.exportRawSeed());
    }
    const {
      format,
      type,

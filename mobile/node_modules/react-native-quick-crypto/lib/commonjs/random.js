...
function getNative() {
  if (random == null) {
    ...
    random = _reactNativeNitroModules.NitroModules.createHybridObject('Random');
  }
  return random;
}
...
function randomBytes(size, callback) {
  ...
  randomFill(buf.buffer, 0, size, (error, res) => {
    if (error) {
      callback(error);
    }
...
let randomCache = new _reactNativeBuffer.Buffer(6 * 1024);
...
function asyncRefillRandomIntCache() {
  ...
  randomFill(randomCache, (err, res) => {
    ...
    if (!err) {
      randomCache = _reactNativeBuffer.Buffer.from(res);
      randomCacheOffset = 0;
    }

...
class EcUtils {
  static get native() {
    if (!this._native) {
      this._native = NitroModules.createHybridObject('EcKeyPair');
    }
    return this._native;
  }
...
export async function ec_generateKeyPairNode(options, encoding) {
  const ec = ec_prepareKeyGenParams(options);
  await ec.generateKeyPair();
  return ec_formatKeyPairOutput(ec, encoding);
}
export function ec_generateKeyPairNodeSync(options, encoding) {
  const ec = ec_prepareKeyGenParams(options);
  ec.generateKeyPairSync();
  return ec_formatKeyPairOutput(ec, encoding);
}
export function ecDeriveBits(algorithm, baseKey, length) {
  ...
  const opensslCurve = kNamedCurveAliases[namedCurve];
